//! This is a Rust implementation of the `Cipher` and `InvCipher`
//! methods in the Advanced Encryption Standard (AES) [FIPS-PUB-197].
...
/// S-box [FIPS-PUB-197 Section 5.1.1, Table 4].
...
/// Inverse S-box [FIPS-PUB-197 Section 5.3.2, Table 6].
...
fn inv_round(mut xs: Block, ki: Block) -> Block {
    ...
    inv_subbytes(inv_shiftrows(inv_mixcolumns(xs)))
}

/// Takes two `Block`s and returns the first `Block` XOR'd with the
/// second.
...
#[must_use]
pub fn cipher(ks: &[Block], plaintext: Block) -> Block {
...
#[allow(clippy::needless_range_loop)]
#[must_use]
pub fn inv_cipher(ks: &[Block], ciphertext: Block) -> Block {
    let pre = inv_subbytes(inv_shiftrows(xorb(ciphertext, ks[ks.len() - 1])));
    let mut mid = pre;
    for i in (1..(ks.len() - 1)).rev() {
